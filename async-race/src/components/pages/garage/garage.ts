...
class Garage {
  ...
  public static createGarageCar({ id, name, color, engineStarted }: Car): string {
    return `
  ...
  public garageContent(cars: Car[], totalCount: string, page: number): HTMLElement {
    const garage = document.querySelector('.garage') as HTMLDivElement;
    garage.innerHTML = `
        <h1 class='garage-title'>Garage (${totalCount})</h1>
        <h2 class='garage-page'>Page #${page}</h2>
        <ul class='garage-cars'>
          ${cars
            .map(
              (car) => `
              <li class="garage-car">${Garage.createGarageCar(car)}</li>
              `,
            )
            .join('')}
        </ul>
